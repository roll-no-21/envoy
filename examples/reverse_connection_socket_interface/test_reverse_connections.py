...
class ReverseConnectionTester:
    ...
    def start_docker_compose(self, on_prem_config: str = None) -> bool:
        ...
        if on_prem_config:
            ...
            # Copy WebSocket service files to temp directory
            websocket_files = ["Dockerfile.websocket", "websocket_server.py"]
            for websocket_file in websocket_files:
                src_file = os.path.join(CONFIG['script_dir'], websocket_file)
                temp_file = os.path.join(self.temp_dir, websocket_file)
    ...
    def test_reverse_websocket_connection(self, port: int) -> bool:
        ...
        try:
            ...
            test_messages = ["Hello WebSocket!", "Testing reverse connection", "Goodbye!"]
            received_messages = []
    ...
    def run_test(self):
        ...
        try:
            ...
            # Test WebSocket request through reverse connection
            logger.info("Testing WebSocket request through reverse connection")
            if not self.test_reverse_websocket_connection(CONFIG['cloud_egress_port']):  # cloud-envoy's egress port
                raise Exception("Reverse connection WebSocket request failed")
            logger.info("✓ Reverse connection WebSocket request successful")
